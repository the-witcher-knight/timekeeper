...
func TestHandler_GetAttendances(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		mockCtrl      mockCtrl
		expStatusCode int
		expRespBody   []attendanceResponse
		expErr        error
	}{
		"success": {
			queryParams: map[string]string{
				...
				outResult: []model.Attendance{
					{
						...
						UpdatedAt:   time.Date(2024, 6, 20, 0, 0, 0, 0, time.UTC),
					},
				},
			},
	...
	for desc, tc := range tcs {
		t.Run(desc, func(t *testing.T) {
			...
			if tc.mockCtrl.isCalled {
				attCtrlMock.On("ListAttendances", r.Context(), tc.mockCtrl.input).
			...
			if tc.expErr != nil {
				...
				var respBody []attendanceResponse
